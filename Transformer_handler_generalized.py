...
class TransformersSeqClassifierHandler(BaseHandler, ABC):
    ...
    def preprocess(self, requests):
        ...
        for idx, data in enumerate(requests):
            ...
            # preprocessing text for question_answering.
                ...
                inputs = self.tokenizer.encode_plus(question, context, max_length=int(max_length), 
    ...
    def inference(self, input_batch):
        ...
        input_ids_batch, token_ids_batch, attention_mask_batch = input_batch
        inferences = []
        ...
        # Handling inference for question_answering.
            ...
            if self.setup_config["save_mode"]=="pretrained":
                ...
                outputs = self.model(torch.tensor([input_ids_batch]), # The tokens representing our input text.
                                    token_type_ids=torch.tensor([token_ids_batch]))

                #token_type_ids=torch.tensor([segment_ids]))
                logger.log(outputs)
                logger.log(outputs.shape)
                answer_start_scores = torch.argmax(outputs[0]) #outputs.start_logits
                answer_end_scores = torch.argmax(outputs[1]) #outputs.end_logits
            else:
